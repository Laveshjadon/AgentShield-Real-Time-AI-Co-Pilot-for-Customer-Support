...
async def api_post(endpoint: str, body: dict) -> dict | None:
    try:
        ...
        logger.error(f"API POST {endpoint} — connection refused (FastAPI not running?)")
        return None
    except httpx.HTTPStatusError as e:
        logger.error(f"API POST {endpoint} — server error {e.response.status_code}: {e.response.text[:200]}")
        return None
...
def create_dashboard():
    ...
    window.AgentShieldAudio = (function() {
        ...
        let browserRecognitionUnavailable = false;
        ...
        function startLiveSpeechRecognition() {
            if (browserRecognitionUnavailable) {
                return;
            }
            ...
            recognition.onerror = function(event) {
                ...
                if (event.error === 'network') {
                    browserRecognitionUnavailable = true;
                    if (recognition) {
                    ...
                    queue.push({
                        type: 'info',
                        message: 'Browser live captions are unavailable. Backend Whisper audio capture is still active.'
                    });
                    return;
                }
                queue.push({
            ...
            recognition.onend = function() {
                if (isRecording && recognition && !browserRecognitionUnavailable) {
                ...
                try {
                    ...
                    await new Promise(function(resolve, reject) {
                        const timeout = setTimeout(function() {
                            reject(new Error('Audio WebSocket connection timed out'));
                        }, 8000);
                        ws.onopen = function() {
                            clearTimeout(timeout);
                            const queued = pendingChunks;
                            pendingChunks = [];
                            queued.forEach(sendChunk);
                            resolve();
                        };
                        ws.onerror = function() {
                            clearTimeout(timeout);
                    ...
                    processor.onaudioprocess = function(e) {
                        ...
                        for (let i = 0; i < input.length; i++) {
                            signalPeak = Math.max(signalPeak, Math.abs(input[i]));
                        }
                    ...
                    signalCheckTimer = setTimeout(function() {
                        if (isRecording && signalPeak < 0.002) {
                            queue.push({
                                type: 'error',
                                message: 'No microphone signal detected. Check the selected Windows input device and browser microphone permission.'
                            });
                        }
                    }, 3000);
    ...
    with ui.footer().style("background:#13182a; border-top:1px solid #2d3748; padding:14px 24px;"):
        with ui.row().classes("w-full items-center gap-3"):
            ...
            async def start_call():
                ...
                if not data:
                    ui.notify(
                        "Failed to start call. Check the terminal for the exact error. "
                        "Common causes: FastAPI still loading (wait ~30s), "
                        "Redis not running (set REDIS_ALLOW_FAKE=1 in .env), "
                        "or PostgreSQL not started (run: docker compose up -d db).",
                        color="negative",
                        close_button=True,
                        timeout=10000,
                    )
            ...
            start_btn.disable()
            start_btn.set_text("LOADING BACKEND...")
                    ...
                    async with httpx.AsyncClient(timeout=2.0) as client:
                        ...
                        if r.status_code == 200:
                            ...
                            if data.get("status") == "running":
                                start_btn.enable()
                                start_btn.set_text("START CALL")
                                ui.notify("Backend connected!", color="positive")
                                health_timer.deactivate()
                except Exception:
                    pass

            health_timer = ui.timer(2.0, check_backend)
