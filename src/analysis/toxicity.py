"""
Toxicity Analyser
scores customer frustration, abuse, and threats in real time.

i built this in two passes to keep latency low:
  pass 1 — fast regex/keyword scan for english + hinglish. no api calls.
  pass 2 — only runs if pass 1 finds something suspicious (score > 0.2),
            then calls the llm to get a proper score and we blend the two.

this way most safe calls never touch the llm at all.
"""
    ...
    def __init__(self):
        ...
        try:
            ...
            if provider == "groq":
                ...
                logger.info("OpenAI LLM connected for deep toxicity analysis.")
            else:
                raise ValueError(f"Unsupported LLM_PROVIDER for toxicity: {provider}")
            self._deep_analysis_chain = self._build_deep_analysis_chain()
        except Exception as e:
            logger.warning(f"LLM not available for deep toxicity analysis: {e}")
            self._llm = None
...
ONLY return the JSON. No other text."""),
    ...
    def _layer1_keyword_scan(self, text: str) -> tuple[float, list]:
        """
        fast keyword scan — no llm involved. checks for threats, abuse, and frustration
        signals in both english and hinglish. returns the raw score and a list of what triggered.
        """
